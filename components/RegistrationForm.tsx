...
const RegistrationRow = ({
  ...
  const eventsToCheckLimit = events
    ? events.filter((e) => !eventsToExclude.includes(e))
    : [];
...
export function RegistrationForm() {
  ...
  const meetEvents = (
    selectedMeet?.events || SWIM_EVENTS.map((e) => `${genderPrefix}${e}`)
  ...
  useEffect(() => {
    ...
    const subscription = form.watch((value) => {
      if (value.registrations) {
        ...
        const validRegistrations = value.registrations.filter(
          (reg: any) =>
            reg.student?.name ||
        ...
        const handler = setTimeout(() => {
          const payload = validRegistrations.map((r: any) => ({
            ...
            student: {
              ...
              seed:
                r.student.seed && r.student.seed > 0
                  ? r.student.seed
                  : undefined,
            },
            events: r.events,
  ...
  }, [form, syncData, hasLoaded, selectedMeetId]);
  ...
  const columnCounts = (registrations || []).reduce(
    (acc, curr) => {
      if (
        ...
        curr.student?.faculty !== selectedFaculty
      )
        return acc;
  ...
  const handlePrint = async () => {
    ...
    try {
      ...
      const currentMeetEvents = (selectedMeet.events || []).filter(
        (e: string) => e.startsWith(genderPrefix),
      );
          ...
          <TableBody>
            ...
            {selectedMeetId &&
              fields.map((field, index) => {
                ...
                if (gender !== activeTab || faculty !== selectedFaculty)
                  return null;
