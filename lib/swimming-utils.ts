 ...
 */
export function assignToHeats(students: any[], lanesPerHeat: number = LANES_PER_HEAT, laneOrder: number[] = DEFAULT_LANE_ORDER) {
  ...
  for (let h = 0; h < numHeats; h++) {
    // Distribute remainder across heats. To balance them, we give +1 to the first 'remainder' heats.
    // Actually, usually in swimming, the later heats are more full.
    // But for "balanced", we'll just distribute them.
    const swimmersInThisHeat = basePerHeat + (h < remainder ? 1 : 0);
    const heatSwimmers = students.slice(studentIdx, studentIdx + swimmersInThisHeat);
    studentIdx += swimmersInThisHeat;

    // Assign to lanes based on laneOrder
    const heatWithLanes = new Array(lanesPerHeat).fill(null);
    heatSwimmers.forEach((s, i) => {
      // Find the lane number from the priority list
      // i is the 0-based index of the swimmer in this heat (sorted by seed/name)
