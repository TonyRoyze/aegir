...
export default function MeetsPage() {
  ...
  const [editEventPointSystems, setEditEventPointSystems] = useState<
    Record<string, number[]>
  >({});
  const [isEditing, setIsEditing] = useState(false);
  const [editDialogOpen, setEditDialogOpen] = useState(false);

  const [newPointSystem, setNewPointSystem] = useState<number[]>([
    ...DEFAULT_POINT_SYSTEM,
  ]);
  const [newEventPointSystems, setNewEventPointSystems] = useState<
    Record<string, number[]>
  >({});
    ...
    <div className="space-y-8 p-6 md:p-8 animate-in fade-in duration-500">
      <div>
        ...
        <p className="text-muted-foreground">
          Manage swimming meets and their events.
        </p>
      </div>
      ...
      <div className="grid gap-8 lg:grid-cols-2">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Set up a new meet with specific events.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            ...
            <div className="space-y-4 pt-2 border-t border-dashed">
              <div>
                <Label className="text-xs uppercase tracking-widest font-black opacity-40">
            ...
            <Button
              ...
              disabled={isCreating || !newMeetName}
              className="w-full"
            >
              {isCreating ? (
        ...
        <div className="space-y-4">
          ...
          {meets === undefined ? (
            ...
            meets.map((meet) => (
              <Card
                key={meet._id}
                className={cn(meet.status === "archived" && "opacity-60")}
              >
                  ...
                  <div className="flex justify-between items-start">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <Button
                        ...
                        size="icon"
                        onClick={() =>
                          handleStatusChange(meet._id, meet.status!)
                        }
                        title={
                          meet.status === "active"
                            ? "Archive Meet"
              ...
              <div>
                ...
                <div className="grid grid-cols-4 gap-2 mt-3">
                  {editPointSystem.map((points, idx) => (
                    <div key={idx} className="space-y-1">
                      <Label
                        htmlFor={`edit-global-points-${idx}`}
                        className="text-[10px] uppercase font-bold text-neutral-400"
                      >
                        Rank {idx + 1}
                      </Label>
      ...
      >
        <DialogContent>
            ...
            {!generatedToken ? (
              ...
              <div className="space-y-2">
                ...
                <p className="text-xs text-muted-foreground">
                  Anyone with this link can view the faculty and event
                  leaderboards without logging in.
                </p>
              </div>
            )}
          </div>
          <DialogFooter>
            <Button
              variant="outline"
              onClick={() => setPublicLinkDialogOpen(false)}
            >
              Close
            </Button>
          </DialogFooter>
